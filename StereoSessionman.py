...
LINESSESSIONLIST = ['practiceSpinIso','practiceSpinLines','practice120SL','finalSpinLinesRun1','finalSpinLinesRun2'] 
#note: possible sessiontypes should all be lowercase
s.possibleSessionTypes = ['lines'] #possible sessiontypes should all be lower case
#SESSIONLIST = ['fccomp','fcconv'] 
#SESSION_TYPE = ISO
#global ISOSESSIONLIST 
#ISOSESSIONLIST = ['isopractice','practice','iso56'] 
...
def setPerSessionValues():
    ...
    if s.sessiontype == 'lines':
        s.tabEnabled = TRUE
        #s.test = TRUE
        #s.dsound = FALSE
        #print 'sessiontype lines'
        #print s.dlabel
        if s.dlabel == 'practice120SL':
            #print 'p120sl'
            s.dimagedir = r'x:\_old\_Aperture\2002-10-01-Stereo-Study\finalSpinLines\\'
            s.numberoftrials = 120
            #print s.dimagedir 
            
        elif s.dlabel in ['finalSpinLinesRun1','finalSpinLinesRun2']:
            s.dimagedir = r'x:\_old\_Aperture\2002-10-01-Stereo-Study\finalSpinLines\\'
        else:
       ...
       #s.dintermissiondelay = 2 
    ...
    elif s.dlabel in ['probefc']:
        s.dimagedir = r'x:\_old\_Aperture\2002-10-01-Stereo-Study\probeFC\\'
        ...
        s.dimagedir = r'x:\_old\_Aperture\2002-10-01-Stereo-Study\imagesrc\\'
        s.sortlist = FALSE 

   # print s.dimagedir
   # print 'end of set persession values'
...
def continuex(labellist): 
    ...
    if labellist ==['']:
        ...
        print 'attempting to run %s, with sessiontype %s, with imagedir %s' %(s.dlabel, s.sessiontype,s.dimagedir)
...
def main():
    ...
    while 1:
        ...
        #it gets set to a non-default value when values are loaded in session.py
                ...
                elif s.sessiontype == 'default':
                    s.sessionlist = SESSIONLIST
                    
                elif s.sessiontype == 'lines':
                    s.sessionlist = LINESSESSIONLIST

...
class Session:
    ...
    def __init__(self, screen=None): #only runs if class instance is created
        ...
        #self.dcalibrationimage   = 'x:\\calibration_OverUnder.png'
        self.dcalibrationimage   = 'x:\\WorkingDirectory\\calibrateScreen_OverUnder.png'
        #self.dcalibrationimage   = r'C:\_research_cleath\Documents-By-Date-and-Study\2002-Multi-Study\images\calibrationimage.png'
        #self.dcalibrationimage   = r'D:\_Shared_Images\calibrationimage.png'
        ...
        self.sessionlist = []
        self.sessiontype = '' #currently only iso or fc. Really should be handled from sessionman...
        self.possibleSessionTypes = []  #this value should be set from sessionman/stereosessionman.py
        ...
        #self.sortlist = TRUE  #useful for testing
        self.sortlist = FALSE
    ...
    def displaymainmenu(self):
        ...
        self._displayMessage(width=300,height=300,yPos=250,displayString=my_string)
    ...
    def getuserdata(self):
        ...
        if not self.dresumed: #this is participant's first time-- need experimenter to enter some info
            ...
            accepted = FALSE
            sessionTypeString = str(self.possibleSessionTypes)
            #to make life easier, have there be a default value:
            self.sessiontype = self.possibleSessionTypes[0]
            while not accepted:
                answer = ''
                ...
                self.sessiontype = inputbox.ask(self.dscreen, "Sessiontype " + sessionTypeString,self.sessiontype).lower() #lower not really necessary since user can only put in lowercase.. but.
                
                #now, display values so experimenter can confirm.
                if self.sessiontype.lower() not in self.possibleSessionTypes:
                ...
                self._displayMessage(width=300,height=250,yPos=200,displayString=my_string)
        ...
        if self.dresumed:
            ...
            if self.sessiontype in ['fc','fc2']:
                ...
                self.calcspinnerpos = FALSE
                #not everyone who uses this code has a pointer attribute!
                if getattr(self,"pointer",0):
    ...
    def _showintermissionscreen(self):
        ...
        #2002-12-20-1340 remainingblocks = ((self.listlen-self.n)/self.dtrialblocksize)
        remainingblocks = float(self.listlen-self.n)/float(self.dtrialblocksize)
        print 'remainingblocks: %d, listlen: %d, n: %d, trialblocksize: %d'%(remainingblocks,self.listlen, self.n, self.dtrialblocksize)
        
        #kludge 2: 
        if self.dlabel == 'practice120SL':
            remainingblocks = remainingblocks + 16
        elif self.dlabel == 'finalSpinLinesRun1':
        ...
        #2002-12-20-1338 if self.DoNotShowEndingScreen or (remainingblocks!=0):
        if self.DoNotShowEndingScreen or (remainingblocks > 0):
            ...
            self._displayMessage(width=500,height=200,yPos=220,displayString=my_string)
            ...
            self._displayMessage(width=500,height=200,yPos=220,displayString=my_string,textColor=(50,255,50))
    ...
    def _savedata(self,silent=FALSE):
        ...
        #2002-11-05-1215 temporary
        #need to convert pointercloset--surfaces are not pickleable.
        #subclassing this and removing pointer functionality was causing errors on code below.
        #the getattr is checked first, returns false if no such attr, and then evaluates second part of condition.
        if (getattr(self,"pointer",FALSE) and self.pointer.newPointer):
            #need to create dir and save new pointer.
            os.mkdir(os.path.join(DATAPATH,'pointerCloset'))
            for n in range(360):
                #pygame.image.save(self.pointer.pointerCloset[n],str(n)+'.bmp')
                #unfortunately pygame won't save .pngs only bmps. we're losing the alpha channel
                myimg =self.pointer.pointerCloset[n]
                ########## Must convert image to something PIL can use.
                mode = "RGBA"
                imageStringForPil = pygame.image.tostring(myimg,mode)
                pilImage = Image.fromstring(mode,myimg.get_size(),imageStringForPil)
                pilImage.save(os.path.join(DATAPATH,os.path.join('pointerCloset',str(n)+'.png'))) 
                ########
                #imageStringForPygame = pilImage.tostring()
                #self.image = pygame.image.fromstring(imageStringForPygame,pilImage.size,mode)
                ##save rendered image
            self.pointer.newPointer = FALSE
        ...
        if not silent:
            ...
            self._displayMessage(width=250,height=250,yPos=300,displayString=my_string)
    ...
    def run(self):
        ...
        while 1:
            ...
            if event.type == KEYDOWN:
                ...
                #elif event.key == K_f:
                #    self.n -= 1
                elif event.key == K_TAB and self.tabEnabled:
                    #display position in imagelist in upper left of screen
                    #fontobject = pygame.font.Font(None,18)
                    fontobject = self.font
                    try:
...
class Pointer(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image, self.rect = load_image(os.path.join(DATAPATH,'pointerfat.bmp'), -1)
        self.original = self.image
        screen = pygame.display.get_surface()
        self.area = screen.get_rect()
        #self.rect.bottomright = self.area.center
        maxx, maxy= self.area.bottomright
        #self.rect.center = self.area.center
        self.rect.center = (maxx/2, (maxy-70)/4)
        print self.rect.center
        self.move = 0
        self.dizzy = 0
        self.calcpos = 999
        self.direction = CLOCKWISE
        self.pointerCloset = range(360)
        self.newPointer = TRUE #set to true if need to create new pointer.
        #and delete or rename pointerCloset directory.
        if os.path.isdir(os.path.join(DATAPATH,'pointerCloset')):
        #does existing pointer closet exist? if so, load the pointers.
        #need to load files into pointer closet:
            self.newPointer = FALSE
            for n in range(360):
            #    self.pointerCloset[n]=pygame.image.load(os.path.join(DATAPATH,os.path.join('pointerCloset',str(n)+'.bmp')))
                mode="RGBA"
                pilImage = Image.open(os.path.join(DATAPATH,os.path.join('pointerCloset',str(n)+'.png'))) 
                ########
                imageStringForPygame = pilImage.tostring()
                self.pointerCloset[n] = pygame.image.fromstring(imageStringForPygame,pilImage.size,mode)
                ##save rendered image
    ...
    def _spin(self):
        ...
        if self.direction == COUNTERCLOCKWISE:
            ...
            if self.dizzy >= 180:
                ...
                self.dizzy = self.dizzy + 180
        #2002-11-05-1149 we will be making a list of 0-359 containing the pointer image for
        #each degree.
        #self.image = pointerCloset[self.dizzy]
        #print self.dizzy        
        if self.newPointer:
            if self.pointerCloset[self.dizzy]==self.dizzy: #no image was generated and saved yet.
                rotate = pygame.transform.rotate
                self.image = rotate(self.original, self.dizzy)
                ########## Must convert image to something PIL can use.
                mode = "RGBA"
                imageStringForPil = pygame.image.tostring(self.image,mode)
                pilImage = Image.fromstring(mode,self.image.get_size(),imageStringForPil)
                ###### now for the code to remove every other line.
                oldXsize, oldYsize = pilImage.size
                overUnderImage = Image.new(mode,(oldXsize,oldYsize/2))
                rangeL=range(0,oldYsize,2)
                newImageRow = 0
                for row in rangeL:
                    overUnderImage.paste(pilImage.crop((0,row,oldXsize,row+1)),(0,newImageRow))
                    newImageRow += 1
                pilImage = overUnderImage
                ######
                imageStringForPygame = pilImage.tostring()
                self.image = pygame.image.fromstring(imageStringForPygame,pilImage.size,mode)
                #save rendered image
                self.pointerCloset[self.dizzy]=self.image
                ########### 
            else:
                self.image = self.pointerCloset[self.dizzy]
        else:
            self.image = self.pointerCloset[self.dizzy]
        #self.image = self.pointerCloset[self.dizzy]
